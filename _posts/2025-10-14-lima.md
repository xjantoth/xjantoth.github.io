---
title: "Lima"
date: 2022-02-21T10:14:12+0100
lastmod: 2022-02-21T10:14:12+0100
draft: false
description: "How to deploy Kubernetes via kubeadm using Lima on macOS, including SSH tunneling to access services via NodePort."
...
The following Lima configuration file deploys a single-node Kubernetes cluster via kubeadm inside a Lima VM. It includes provisioning scripts that install kubeadm, kubelet, kubectl, and set up flannel as the pod network. It also handles corporate proxy CA certificates if needed.
...
To access a Kubernetes service exposed via NodePort from your macOS host, create an SSH tunnel through the Lima VM. First, retrieve the SSH connection details, then set up port forwarding.

```bash
limactl show-ssh k8s
...

# Create SSH tunnel to NodePort
...
By default, the Kubernetes Dashboard service account has limited permissions. Create a RoleBinding to grant it the `view` ClusterRole so you can browse resources in the dashboard.
